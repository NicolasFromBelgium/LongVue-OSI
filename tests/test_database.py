...
def test_database_schema_exists(db_connection):
    ...
    col_dict = {col[1]: {'type': col[2], 'notnull': col[3], 'default': col[4], 'pk': col[5]} for col in columns}
    ...
    for col, props in expected_columns.items():
        ...
        for prop, value in props.items():
            assert col_dict[col][prop] == value, f"Column '{col}' has incorrect {prop}: got {col_dict[col][prop]}, expected {value}."
    ...
    assert any('sqlite_autoindex_osint_data' in idx for idx in indexes), "No primary key index on uuid."
...
def test_insert_and_retrieve_data(db_connection):
    ...
    """, (sample_uuid, sample_title, sample_url, sample_source, sample_scraper_agent,
          sample_status, sample_http_status, sample_response_time_ms))
